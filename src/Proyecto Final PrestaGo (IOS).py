...
def ruta_salida(nombre_archivo):
    carpeta = os.path.join(os.path.expanduser("~"), "Documents", "PrestaGo")
...
def registrar_prestamo(items, prestamos, usuarios):
    ...
    print("  Puede ingresar una fecha.")
    while True:
        ...
        if len(partes) != 3:
            ...
            print("  Error: La fecha solo puede contener numeros.")
             ...
             print("  Error: El año debe tener exactamente 4 digitos. Ejemplo: 2026")
...
def registrar_devolucion(items, prestamos, usuarios, devoluciones):
    ...
    if len(prestamos_en_tiempo) == 0:
        print("  El prestamo del usuario supera los 30 dias.")
    ...
    if en_tiempo:
        ...
        contenido_cert += "\n" + "=" * 50 + "\n"
        archivo = open(ruta_salida(nombre_txt), "w", encoding="utf-8")
...
def generar_venta(usuario, prestamo, dias_prestado, items, ventas):
    ...
    contenido_fact += "=" * 50 + "\n"
    archivo = open(ruta_salida(nombre_txt_fact), "w", encoding="utf-8")
